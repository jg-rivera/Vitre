...
$(document).ready(function() {
  $table = $('#dataTable').DataTable({
        ...
        "serverSide": true,
        "ajax": {
			"url": "/admin/ajax/load_table_subjects.php",
			"type": "post"
		},
		"columnDefs": [ 
			{
	...
	$table.on('click', '.act-info', function() {
		...
		$('.modal-body').load('/admin/ajax/modal_subjects_info.php?id=' + $sid,function(){
			$('#subjectInfoModal').modal( {
				show: true
			});
		});
	});
	...
	$table.on('click', '.act-delete', function() {
		...
		$.ajax({
			type: "post",
			url: "/admin/ajax/delete_subject.php",
			data: { 
				id: $sid
			},
	...
	$(".act-register").click(function() {
		$('.modal-body').load('/admin/ajax/modal_subjects_registry.php',function(){
			$('#subjectRegistryModal').modal( {
				show: true
			});
		});
	});
	...
	$(".act-register-confirm").click(function() {
		var $header = $("#subjectRegistryModal .modal-dialog .modal-content .modal-body");
		...
		$.ajax({
			type: "post",
			url: "/admin/ajax/register_subject.php",
			data: { 
				name: $name,
				icon_name: $icon_name,
			...
			error: function(result) {
				alert('Error in registering subject.');
			}
		});
	});
});
